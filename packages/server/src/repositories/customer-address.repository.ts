    ...
    try {
      const queryResult = await this.pool.query(/* sql */ `
        ...
        FROM
            customer c
        JOIN
            customer_address a ON c.id = a.customer_id
        order by distance;
      `)
  ...
  async create(customerAddress: CustomerAddress): Promise<Customer> {
    ...
    try {
      ...
      const customerAddressQuery = {
        text: `
          INSERT INTO customer_address (id, customer_id, street, number, neighborhood, city, state, postal_code, latitude, longitude)
        ...
        values: [
          ...
          customerAddress.latitude.toFixed(8),
          customerAddress.longitude.toFixed(8),
        ],
      }
